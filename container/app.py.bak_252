...
APP_VERSION = "2.52 (Premium-Layout-Engine)"
...
def save_gcs_session(data):
    sid = get_sid()
    local_path = SESSIONS_DIR / f"{sid}.json"
    with open(local_path, 'w') as f: json.dump(data, f)
    storage_save(local_path, f"sessions/{sid}.json")

def update_gcs_session(updates):
    data = load_gcs_session()
    data.update(updates)
    save_gcs_session(data)
...
def create_footer(sec, nat_code: str, dil_code: str = "", qualitat: str = "atenea"):
    ...
    # Crear nou paràgraf amb format de tabuladors
    p = ft.add_paragraph()
    p.paragraph_format.tab_stops.add_tab_stop(Inches(3.25), WD_TAB_ALIGNMENT.CENTER)
    p.paragraph_format.tab_stops.add_tab_stop(Inches(6.5), WD_TAB_ALIGNMENT.RIGHT)
    
    # Construir text esquerre
    parts = []
    # Recuperem valors de forma segura
    r_nat = str(nat_code).strip() if nat_code else ""
    r_dil = str(dil_code).strip() if dil_code else ""
    
    if qualitat == 'vector':
        if r_dil: parts.append(f"Dil. {r_dil}")
        if r_nat: parts.append(f"NAT {r_nat}")
    else:
        if r_nat: parts.append(f"NAT {r_nat}")
        if r_dil: parts.append(f"Dil. {r_dil}")
    
    # Si no hi ha res, posem un placeholder per debugging o espai
    left_text = " - ".join(parts) if parts else "Dades no disponibles"
    
    # Afegeix contingut: Esquerra [TAB] Centre (buit) [TAB] Dreta
    r1 = p.add_run(left_text)
    r1.bold = True; r1.font.size = Pt(8); r1.font.name = "Arial"
    
    p.add_run("\t") # Tab al centre
    
    p_txt = f"ART MNORD"
    r2 = p.add_run(p_txt)
    r2.bold = True; r2.font.size = Pt(8); r2.font.name = "Arial"
    
    p.add_run("\t") # Tab a la dreta
    
    # Paginació
    p_pag = p.add_run("Pàg. ")
    p_pag.font.size = Pt(8)
    add_field(p, 'PAGE')
...
def add_logo_to_header(header):
    try:
        ...
        # --- MOTOR DE LOGO ABSOLUT (BASE64) ---
        import base64
        
        try:
            # Decodificar el logo des de la constant de codi
            logo_data = base64.b64decode(LOGO_PMA_B64)
            img_stream = io.BytesIO(logo_data)
            
            # Creem una taula d'una sola cel·la alineada a l'esquerra per fixar el logo
            # Això és el mètode més estable en Word
            table = header.add_table(1, 1, width=Inches(5))
            table.alignment = WD_TABLE_ALIGNMENT.LEFT
            cell = table.rows[0].cells[0]
            cell.vertical_alignment = WD_ALIGN_VERTICAL.CENTER
            
            # Netejar paràgraf de la cel·la i inserir imatge
            p_cell = cell.paragraphs[0]
            p_cell.clear()
            run = p_cell.add_run()
            # 3.2 polzades per al logo combinat es veu nítid i professional sense ocupar massa espai
            run.add_picture(img_stream, width=Inches(3.2))
            print(f"HEADER: Logotip Base64 inserit amb èxit (v{APP_VERSION})")
            
        except Exception as b64e:
            p.add_run(f"ERROR_V252: {b64e}").font.size = Pt(8)
            print(f"HEADER FAIL: Error decodificant B64: {b64e}")
                
    except Exception as e:
        print(f"ERROR_HEADER_PRO: {e}")

def add_photo_block(doc, img_buffer, photo_num, max_w_cm, max_h_cm, description=""):
    img_buffer.seek(0)
    ...
    p_caption = doc.add_paragraph(); p_caption.alignment = Align.JUSTIFY
    p_caption.paragraph_format.space_before = Pt(0); p_caption.paragraph_format.space_after = Pt(2)
    r1 = p_caption.add_run(f"Fotografia núm. {photo_num}"); r1.bold = True; r1.font.size = Pt(9); r1.font.name = 'Arial'
    if description:
        r2 = p_caption.add_run(f": {description}")
        r2.font.size = Pt(9); r2.font.name = 'Arial'
    else:
        p_caption.add_run(": ").font.size = Pt(9)
...
def generate_ai_descriptions(evolucio: str, image_files: List[str]) -> Dict[str, str]:
    if not image_files: return {}
    
    print(f"DEBUG: IA - Iniciant generació Studio per {len(image_files)} imatges. Relat: {bool(evolucio)}")
    init_google_ai()
    ...
    prompt_text = (
        ...
        f"CONTEXT EXPLICU DE L'ACCIDENT: '{context_evolucio}'.\n\n"
        ...
        "5. RIFLEXIÓ I PROFUNDITAT: No siguis telegràfic. Redacta paràgrafs detallats, exhaustius i amb substància fòrense. La descripció ha de ser un bloc de text continu de 3 a 5 frases amb una densitat informativa màxima, digna d'un pèrit judicial de carrera.\n\n"
    ...
    for idx, fname in enumerate(image_files):
        update_status(f"Processant imatges ({idx+1}/{total_imgs})...")
        try:
            ...
            if not src_path and IS_PROD:
                for folder_name in ["work", "master", "edited"]:
                    ...
                    print(f"DEBUG: IA - Intentant descarregar {fname} de {folder_name}...")
                    if storage_download(f"uploads/{folder_name}/{fname}", p_target):
                        src_path = p_target
                        print(f"DEBUG: IA - Descarregada correctament de {folder_name}")
                        break
            ...
            if src_path:
             ...
             print(f"DEBUG: IA Error imatge {fname}: {e}")
    ...
    for model_name in models_to_try:
        try:
            print(f"DEBUG: IA - Intentant amb {model_name}...")
            ...
            if response and response.text:
                print(f"DEBUG: IA - Èxit amb {model_name}. Processant JSON...")
                try:
                    raw_data = json.loads(response.text)
                except:
                    print(f"DEBUG: IA - JSON Invàlid: {response.text}")
                    continue
                ...
                print(f"DEBUG: IA Claus Retornades: {ai_keys}")
                print(f"DEBUG: Imatges Esperades: {image_files}")
                
                for i, fname in enumerate(image_files):
                    # 2a. Coincidència exacta
                    if fname in temp_dict:
                        final_dict[fname] = temp_dict[fname]
                    # 2b. Coincidència parcial (si la IA es deixa .jpg)
                    elif fname.split('.')[0] in temp_dict:
                         final_dict[fname] = temp_dict[fname.split('.')[0]]
                    # 2c. Fallback per índex (si la IA retorna claus rares però en ordre)
                    elif i < len(ai_keys):
                        # Assumim que l'ordre es respecta
                        key_at_index = ai_keys[i]
                        final_dict[fname] = temp_dict[key_at_index]
                
                return final_dict
                
                # Assegurar que tenim alguna cosa
                if not final_dict:
                    print("ADVERTÈNCIA: IA ha retornat JSON vàlid però buit o format desconegut.")
                    return {f: "Error de format IA" for f in image_files}
    ...
    return {f: "Error crític de connexió amb Gemini 3 (Revisa la Clau API)" for f in image_files}
...
def generate_api():
    ...
    def bg_task(sid, evol, imgs, manuals):
        try:
            ...
            ai_descs = generate_ai_descriptions(evol, imgs)
...
def create_report():
    try:
        ...
        page_h_cm, page_w_cm, margin_cm = 29.7, 21.0, 1.8
        ...
        p_dest.runs[0].font.size = Pt(21); p_dest.runs[0].font.name = 'Arial'
        doc.add_paragraph()
        
        usable_w_cm = page_w_cm - (margin_cm * 2); usable_h_cm = page_h_cm - (margin_cm * 2)
        photo_counter = 1; i = 0
        while i < len(processed_images):
            ...
            # Forcem logo a cada secció per seguretat màxima
            add_logo_to_header(sec.header); create_footer(sec, nat_code, dil_code, qualitat)
            set_vertical_alignment(sec, 'center')
            
            # Forçar 2 fotos per pàgina (màxim 9.5cm alçada cadascuna)
            for _slot in range(2):
                if i < len(processed_images):
                    desc = final_descriptions.get(images_ordered[i], "")
                    # Limitem a 9.2cm per garantir que 2 caben bé amb el logo i footer
                    add_photo_block(doc, processed_images[i], photo_counter, usable_w_cm, 9.2, desc)
                    photo_counter += 1
                    i += 1
                    if _slot == 0 and i < len(processed_images):
...
def update_status(message):
    ...
    update_gcs_session({'status_message': message})
