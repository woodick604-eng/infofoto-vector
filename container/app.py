...
APP_VERSION = os.environ.get("APP_VERSION", "2.74")
...
# --- GENERATION ---
def init_google_ai():
    api_key = os.environ.get("GEMINI_API_KEY", "AIzaSyBv2VlQXXpVLjDXsSX2nnz9jOsFhMv2msw")
...
def login():
    if request.method == "POST":
        ...
        if pin == "5085":
            ...
            return render_template("login.html", error="PIN Incorrecte")
    return render_template("login.html")

@app.before_request
def require_login():
    # Exclusions white-list
    if request.endpoint == 'login' or (request.endpoint and request.endpoint.startswith('static')): return
    if request.path.startswith('/static'): return # Extra safety
    
    if not session.get('authenticated'):
        print(f"DEBUG: Accés no autoritzat a {request.path}. Redirigint a Login.")
        return redirect(url_for('login'))

@app.get("/")
def index():
    # Netejar dades de sessió anteriors PERÒ mantenir l'autenticació
