...
APP_VERSION = os.environ.get("APP_VERSION", "2.60 (Final)")
...
def create_footer(sec, nat_code: str, dil_code: str = "", qualitat: str = "atenea"):
    ...
    # Crear nou paràgraf amb format de tabuladors (Només a la dreta per a paginació)
    p = ft.add_paragraph()
    p.paragraph_format.tab_stops.add_tab_stop(Inches(6.5), WD_TAB_ALIGNMENT.RIGHT)
    
    # Construir text: NAT - ARTMNORD - Diligències
    # "NAT XXXX/XX va ARTMNORD ... luego seguido de las diligencias"
    parts_line = []
...
def add_logo_to_header(header):
    try:
        ...
        if not header.paragraphs:
            ...
            p.clear() # Netejar qualsevol text previ
            
        # Utilitzem una taula per aliniació vertical perfecta
        # Taula 1x1, sense vores per defecte
        tbl = header.add_table(rows=1, cols=1, width=Inches(6))
        tbl.alignment = WD_TABLE_ALIGNMENT.LEFT
        cell = tbl.rows[0].cells[0]
        cell.vertical_alignment = WD_ALIGN_VERTICAL.CENTER
        
        # Netejar paràgraf de la cel·la
        p_cell = cell.paragraphs[0]
        p_cell.alignment = Align.LEFT
        run = p_cell.add_run()
        ...
        # Mida "EL DOBLE" (era 1.5 -> 3.0 polzades)
        header_logo_width = Inches(3.0) 
